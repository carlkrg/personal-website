const cities = {
  Lisbon: {
    latitude: 38.7223,
    longitude: -9.1393,
    logos: ["static/img/nova-logo.svg"],
  },
  Berlin: {
    latitude: 52.52,
    longitude: 13.405,
    logos: [
      "static/img/hwr-logo.svg",
      "static/img/KPMG-logo.svg",
      "static/img/tu-logo.svg",
    ],
  },
  Shanghai: {
    latitude: 31.2304,
    longitude: 121.4737,
    logos: ["static/img/dss-logo.svg"],
  },
  Bonn: { latitude: 50.7374, longitude: 7.0982, logos: [] },
  "New Delhi": {
    latitude: 28.6139,
    longitude: 77.209,
    logos: ["static/img/DSND-logo.svg"],
  },
};
...
function drawMap() {
  ...
  d3.json("static/img/custom.geo.json").then(function (data) {
    ...
    svg
      ...
      .attr(
        "x",
        (d) => projection([d[1].longitude, d[1].latitude])[0] - markerWidth / 2
      )
      .attr(
        "y",
        (d) => projection([d[1].longitude, d[1].latitude])[1] - markerHeight
      )
      ...
      .on("mouseover", function (event, d) {
        ...
        d[1].logos.forEach((logo, index) => {
          ...
          const logoY =
            ...
            30 +
            index * logoSpacing -
            (d[1].logos.length * logoSpacing) / 2; // Center logos vertically around marker
      ...
      .on("mouseout", function () {
        d3.select(this).attr("width", markerWidth).attr("height", markerHeight);
        svg.selectAll(".city-logo").remove(); // Remove logos
      });
    // Append permanent labels for each city
    svg
      .selectAll(".city-label")
      .data(Object.entries(cities))
      .enter()
      .append("text")
      .attr("x", (d) => projection([d[1].longitude, d[1].latitude])[0])
      .attr(
        "y",
        (d) => projection([d[1].longitude, d[1].latitude])[1] - markerHeight
      )
